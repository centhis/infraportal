...
from app.settings.ldap.services import update_ldap_setting, get_ldap_setting_by_key
from app.users.ldap.services import connect_to_ldap, authenticate_ldap_user, get_ldap_user_info, normalize_object_guid
    ...
    for key, (type_str, value) in settings_to_set.items():
        ...
        if not db_setting:
            db_setting = LdapSetting(
                ...
                value=str(value),
                type=type_str,
                is_sensitive=(key == "LDAP_BIND_PASSWORD")
            )
...
class MockLdapEntry:
    ...
    def __getattr__(self, name):
        if name in self._attributes:
            ...
            # Add raw_values for objectGUID testing
            if name == 'objectGUID':
                mock.raw_values = [self._attributes[name]]
            return mock
        raise AttributeError(name)

def create_mock_connection(server_type="ad", user_found=True, bind_success=True, object_guid_format="bytes"):
...
def test_connect_to_ldap_ad(db_session, ldap_settings):
    ...
    with patch("app.users.ldap.services.Server", return_value=mock_server), \
         patch("app.users.ldap.services.Connection", return_value=mock_conn):
...
def test_authenticate_ldap_user_success(db_session, ldap_settings):
    ...
    mock_conn, mock_server = create_mock_connection(server_type="ad")
    with patch("app.users.ldap.services.Server", return_value=mock_server), \
         patch("app.users.ldap.services.Connection", side_effect=[mock_conn, mock_conn]):
...
def test_authenticate_ldap_user_fail_bind(db_session, ldap_settings):
    ...
    with patch("app.users.ldap.services.Server", return_value=mock_server), \
         patch("app.users.ldap.services.Connection", side_effect=[mock_admin_conn, mock_user_conn]):
...
def test_authenticate_ldap_user_with_complex_filter(db_session, ldap_settings):
    ...
    """
    update_ldap_setting(db_session, "LDAP_USER_FILTER", "(&(objectClass=user)(memberOf=CN=group,DC=test,DC=local))")
    ...
    with patch("app.users.ldap.services.Server", return_value=mock_server), \
        ...
        expected_filter = "(&(objectClass=user)(memberOf=CN=group,DC=test,DC=local)(sAMAccountName=ldapuser))"
