...
def test_get_task_secret_success(internal_client, db_session: Session):
    ...
    setting = LdapSetting(key="LDAP_BIND_PASSWORD", value=encrypted_pass, type="string", is_sensitive=True)

    mapping = TaskSecretMapping(task_type="users:sync_ldap", setting_key="LdapSetting:LDAP_BIND_PASSWORD")
    db_session.add_all([setting, mapping])
    db_session.commit()
    
    response = internal_client.post(
        "/api/internal/settings/secrets",
        json={"task_type": "users:sync_ldap"}
    )
...
def test_get_task_secret_forbidden(internal_client, db_session: Session):
    ...
    response = internal_client.post(
        "/api/internal/settings/secrets",
        json={"task_type": "unauthorized_task"}
    )
