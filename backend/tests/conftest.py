...
from app.core.security import ACCESS_TOKEN_EXPIRE, REFRESH_TOKEN_EXPIRE, create_token
...
def non_admin_client(test_app_client_factory, user_factory):  # Use the factory
    ...
    login_response = client.post(
        "/api/v1/auth/login", json={"login": "regular_user_rbac", "password": "password_rbac"}
    )
    access_token = login_response.json()["access_token"]
    client.headers["Authorization"] = f"Bearer {access_token}"
    return client
...
def tmp_role(db_session, tmp_permission):
    ...
    def _tmp_role(
        name: str = "test_role",
        description: str = "A temporary role",
        ...
        if permissions:
            for perm_name in permissions:
                permission = (
                    db_session.query(Permission).filter(Permission.name == perm_name).first()
                )
                if not permission:
...
def user_factory(db_session, tmp_group, tmp_role, tmp_permission):  # Added tmp_permission
    ...
    def _user_factory(
        ...
        roles: list[
            dict
        ] = None,  # Expects a list of dicts: [{"name": "role1", "permissions": ["perm1"]}]
    ):
        ...
        if roles:
            ...
            if not user_groups:  # If user is not already assigned to any group, create one
                ...
                if not roles_group:
                    roles_group = tmp_group(
                        name=roles_group_name, description=f"Roles group for {login}"
                    )
