...
def test_delete_built_in_group(authenticated_client: TestClient):
    admins_group = authenticated_client.get("/api/v1/groups?skip=0&limit=1").json()["items"][0]
...
def test_delete_built_in_role(authenticated_client: TestClient):
    admin_role = authenticated_client.get("/api/v1/roles?skip=0&limit=1").json()["items"][0]
...
def test_update_built_in_user_login(authenticated_client: TestClient):
    ...
    response = authenticated_client.put(
        f"/api/v1/users/{admin_user['id']}", json={"login": "newlogin"}
    )
    assert response.status_code == 400
...
def test_update_built_in_user_name(authenticated_client: TestClient):
    admin_user = authenticated_client.get("/api/v1/users/by-login/admin").json()
    response = authenticated_client.put(
        f"/api/v1/users/{admin_user['id']}", json={"name": "newname"}
    )
    assert response.status_code == 400
...
def test_update_built_in_group_name(authenticated_client: TestClient):
    admins_group = authenticated_client.get("/api/v1/groups?skip=0&limit=1").json()["items"][0]
    response = authenticated_client.put(
        f"/api/v1/groups/{admins_group['id']}", json={"name": "newname"}
    )
    assert response.status_code == 400
...
def test_update_built_in_role_name(authenticated_client: TestClient):
    admin_role = authenticated_client.get("/api/v1/roles?skip=0&limit=1").json()["items"][0]
    response = authenticated_client.put(
        f"/api/v1/roles/{admin_role['id']}", json={"name": "newname"}
    )
    assert response.status_code == 400
...
def test_create_group(authenticated_client: TestClient):
    response = authenticated_client.post(
        "/api/v1/groups/", json={"name": "testgroup", "description": "Test group"}
    )
...
def test_create_role(authenticated_client: TestClient):
    response = authenticated_client.post(
        "/api/v1/roles/", json={"name": "testrole", "description": "Test role"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_unauthorized_create_group(non_admin_client: TestClient):
    response = non_admin_client.post(
        "/api/v1/groups/", json={"name": "forbidden_group", "description": "Forbidden group"}
    )
...
def test_unauthorized_update_group(authenticated_client: TestClient, non_admin_client: TestClient):
    ...
    create_response = authenticated_client.post(
        "/api/v1/groups/", json={"name": "updatable_group", "description": "Group to update"}
    )
...
def test_unauthorized_delete_group(authenticated_client: TestClient, non_admin_client: TestClient):
    # First, create a non-built-in group as an admin
    create_response = authenticated_client.post(
        "/api/v1/groups/", json={"name": "deletable_group", "description": "Group to delete"}
    )
    assert create_response.status_code == 200  # Admin should be able to create
    group_id = create_response.json()["id"]
...
def test_unauthorized_create_role(non_admin_client: TestClient):
    response = non_admin_client.post(
        "/api/v1/roles/", json={"name": "forbidden_role", "description": "Forbidden role"}
    )
    assert response.status_code == 403
    assert response.json()["detail"] == "You do not have permission to perform this action"
...
def test_unauthorized_update_role(authenticated_client: TestClient, non_admin_client: TestClient):
    ...
    create_response = authenticated_client.post(
        "/api/v1/roles/", json={"name": "updatable_role", "description": "Role to update"}
    )
    assert create_response.status_code == 200  # Admin should be able to create
...
def test_unauthorized_delete_role(authenticated_client: TestClient, non_admin_client: TestClient):
    # First, create a non-built-in role as an admin
    create_response = authenticated_client.post(
        "/api/v1/roles/", json={"name": "deletable_role", "description": "Role to delete"}
    )
    assert create_response.status_code == 200  # Admin should be able to create
    role_id = create_response.json()["id"]
...
def test_assign_permissions_to_role(db_session, tmp_role, tmp_permission):
    ...
    created_nonexistent_permission = (
        db_session.query(Permission).filter(Permission.name == nonexistent_perm_name).first()
    )
...
def test_assign_roles_to_group_and_check_non_existent_role(
    authenticated_client: TestClient, tmp_group, tmp_role
):
    """
    ...
    update_group_with_bad_role_response = authenticated_client.put(
        f"/api/v1/groups/{created_group_id}",
        json={"roles": [created_role_id, non_existent_role_id]},
    )
    ...
    assert not any(
        role["id"] == non_existent_role_id for role in updated_group_with_bad_role_data["roles"]
    )
    assert any(
        role["id"] == created_role_id for role in updated_group_with_bad_role_data["roles"]
    )  # Original role should still be there


def test_assign_groups_to_user_and_check_non_existent_group(
    authenticated_client: TestClient, user_factory, tmp_group
):
    """
    ...
    create_user_response = authenticated_client.post(
        "/api/v1/users/",
        json={"login": user_login, "password": "password", "name": "User with Groups"},
    )
    ...
    update_user_with_bad_group_response = authenticated_client.put(
        ...
        json={"group_ids": [created_group_id, non_existent_group_id]},
    )
    ...
    assert (
        "One or more group IDs are invalid." in update_user_with_bad_group_response.json()["detail"]
    )


def test_permission_inheritance_through_groups_and_roles(
    authenticated_client: TestClient, user_factory, tmp_group, tmp_role, tmp_permission, db_session
):
    """
    ...
    viewer_user = user_factory(
        login=viewer_user_login,
        password=viewer_password,
        name="Viewer User",
        groups=[viewer_group_name],
    )

    # 5. Log in as this user to get an access_token
    login_response = authenticated_client.post(
        "/api/v1/auth/login", json={"login": viewer_user_login, "password": viewer_password}
    )
    ...
    #    and verify access.
    permissions_report_response = viewer_client.get(
        f"/api/v1/users/{viewer_user.id}/permissions_report"
    )
    ...
    assert "items" in list_users_response.json()
    ...
    authenticated_client.put(
        f"/api/v1/groups/{creator_group.id}", json={"roles": [creator_role.id]}
    )
    ...
    creator_user = user_factory(
        login=creator_user_login,
        password=creator_password,
        name="Creator User",
        groups=[creator_group_name],
    )

    # 5. Log in as this user to get an access_token
    login_response_creator = authenticated_client.post(
        "/api/v1/auth/login", json={"login": creator_user_login, "password": creator_password}
    )
    ...
    assert (
        list_users_response_creator.json()["detail"]
    ...
    permissions_report_response_creator = creator_client.get(
        f"/api/v1/users/{creator_user.id}/permissions_report"
    )
    assert permissions_report_response_creator.status_code == 403
    assert (
        permissions_report_response_creator.json()["detail"]
        == "You do not have permission to perform this action"
    )
...
def test_create_group_with_empty_name(authenticated_client: TestClient):
    ...
    """
    response = authenticated_client.post(
        "/api/v1/groups/", json={"name": "", "description": "Group with empty name"}
    )
...
def test_create_group_with_missing_name(authenticated_client: TestClient):
    ...
    """
    response = authenticated_client.post(
        "/api/v1/groups/", json={"description": "Group with missing name"}
    )
    assert response.status_code == 422
    assert "name" in response.json()["detail"][0]["loc"]
...
def test_create_role_with_empty_name(authenticated_client: TestClient):
    ...
    """
    response = authenticated_client.post(
        "/api/v1/roles/", json={"name": "", "description": "Role with empty name"}
    )
    assert response.status_code == 422
    assert "name" in response.json()["detail"][0]["loc"]
    assert "String should have at least 3 characters" in response.json()["detail"][0]["msg"]
...
def test_create_role_with_missing_name(authenticated_client: TestClient):
    ...
    """
    response = authenticated_client.post(
        "/api/v1/roles/", json={"description": "Role with missing name"}
    )
    assert response.status_code == 422
    assert "name" in response.json()["detail"][0]["loc"]
    assert "Field required" in response.json()["detail"][0]["msg"]
