...
def test_apply_patches_success():
    ...
    with patch("celery_sqlalchemy_scheduler.models", mock_models), \
...
def test_apply_patches_exception_handling():
    with patch("celery_sqlalchemy_scheduler.models.PeriodicTaskChanged.update_changed", side_effect=Exception("error")):
        # Should not raise
        apply_patches()
