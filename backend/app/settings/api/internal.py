...
# Реестр маппинга Имя Модели -> Функция Геттер
SETTINGS_PROVIDERS = {"LdapSetting": ldap_services.get_ldap_setting_value}

router = APIRouter(tags=["Internal Settings"])


@router.post("/settings/secrets", dependencies=[Depends(get_worker_api_key)])
def get_task_secret(task_type: str = Body(..., embed=True), db: Session = Depends(get_db)):
    """
    Внутренний эндпоинт для получения секретов задачи.
    Возвращает словарь всех разрешенных секретов.
    """
    # 1. Найти все разрешенные ключи
