...
from app.users.local.schemas import (
    GroupWithRolesAndPermissionsSchema,
    RoleWithPermissionsSchema,
    UserPermissionsReportSchema,
)  # Импортированы новые схемы
...
class PermissionService:
    ...
    def get_user_permissions_report(self, user_id: int) -> UserPermissionsReportSchema:
        ...
        user_orm = (
            self.db.query(User)
            .options(joinedload(User.groups).joinedload(Group.roles).joinedload(Role.permissions))
        ...
        for group_orm in user_orm.groups:
            ...
            for role_orm in group_orm.roles:
                ...
                for perm_orm in role_orm.permissions:
                    ...
                    all_unique_permissions_map[perm_schema.id] = (
                        perm_schema  # Добавляем в карту уникальных
                    )
        ...
        all_unique_permissions_list = sorted(
            all_unique_permissions_map.values(), key=lambda p: p.name
        )
