...
def list_users(service: UserService = Depends(), skip: int = 0, limit: int = 100):
...
def get_user_by_lofin(login: str, service: UserService = Depends()):
    return service.get_user_by_login(login)

@router.post("/", response_model=UserResponseSchema)
def create_user(data: CreateUserSchema, service: UserService = Depends()):
    return service.create_user(data)

@router.put("/{user_id}", response_model=UserResponseSchema)
def update_user(user_id: int, data: UpdateUserSchema, service: UserService = Depends()):
