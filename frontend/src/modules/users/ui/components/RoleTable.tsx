...
export interface RoleTableProps {
    ...
    onFiltersChange: Dispatch<SetStateAction<RoleTableFilters>>;
...
/**
 * Таблица ролей с серверной пагинацией
 */
export const RoleTable = memo(function RoleTable({
    ...
    const handleFilterChange = useCallback((field: keyof RoleTableFilters, value: string) => {
        onFiltersChange((prev: RoleTableFilters) => ({
            ...prev,
            [field]: value,
        }));
    }, [onFiltersChange]);
    ...
    const columns: GridColDef<Role>[] = useMemo(
        () => [
            {
                field: 'id',
                headerName: t('user_management.roles.table.id'),
                width: 90,
                renderHeader: (params) => (
                    <ColumnFilterHeader
            ...
            {
                field: 'name',
                headerName: t('user_management.roles.table.name'),
                flex: 1,
                renderHeader: (params) => (
                    <ColumnFilterHeader
                        label={params.colDef.headerName ?? ''}
            ...
            {
                field: 'description',
                headerName: t('user_management.roles.table.description'),
                flex: 2,
                renderHeader: (params) => (
                    <ColumnFilterHeader
                        label={params.colDef.headerName ?? ''}
            ...
            },
            {
                ...
                flex: 1,
                sortable: false,
                renderCell: (params) => <span>{params.row.permissions?.length ?? 0}</span>,
            },
            {
                field: 'built_in',
                ...
                renderHeader: (params) => (
                    <ColumnSelectFilterHeader
                        ...
                        options={[
                            { value: 'true', label: t('user_management.roles.table.type_built_in') },
                            { value: 'false', label: t('user_management.roles.table.type_custom') },
                        ]}
            ...
            {
                field: 'created_at',
                headerName: t('user_management.roles.table.created_at'),
                flex: 1,
