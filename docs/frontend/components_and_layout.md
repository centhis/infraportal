...
*   **Интерфейс**: Элементы в списках представлены в виде `Chip`-компонентов. Перенос элемента из одного списка в другой осуществляется прямым кликом по нему.
...
*   **Применение**: В проекте используется в форме создания/редактирования роли для назначения разрешений.
